 ...
 * REAL `@lanshi17/dsh-sdk-client`, drives one turn over stdio JSON-RPC,
...
} from '@lanshi17/dsh-acp-snapshot'
import { resolveExampleLaunch } from '@lanshi17/dsh-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@lanshi17/dsh-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@lanshi17/dsh-system-prompt') return []
