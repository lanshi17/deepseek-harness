...
Move the basic implementation into `@lanshi17/dsh-compaction` and remove `@lanshi17/dsh-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@lanshi17/dsh-compaction-basic` and its workspace/package metadata are removed.
- `@lanshi17/dsh-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
