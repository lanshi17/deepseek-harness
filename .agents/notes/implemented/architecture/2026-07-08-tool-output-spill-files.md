...
| Package | Role |
|---|---|
| `@lanshi17/dsh-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@lanshi17/dsh-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@lanshi17/dsh-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@lanshi17/dsh-spill-local'

- id: spill-policy
  name: '@lanshi17/dsh-spill-policy'
  config:
...
- `@lanshi17/dsh-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@lanshi17/dsh-spill` owns saving final text and returning a locator plus retrieval hint.
- `@lanshi17/dsh-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
