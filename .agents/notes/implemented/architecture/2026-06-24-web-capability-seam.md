...
1. `@lanshi17/dsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@lanshi17/dsh-web-search-exa`, `@lanshi17/dsh-web-search-perplexity`, `@lanshi17/dsh-web-search-deepseek`, and `@lanshi17/dsh-web-fetch-http`.
3. `@lanshi17/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@lanshi17/dsh-tool-web  --depends on-->  @lanshi17/dsh-web  <--depends on--  @lanshi17/dsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @lanshi17/dsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @lanshi17/dsh-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @lanshi17/dsh-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@lanshi17/dsh-web-search-exa"] -->|registerSearchProvider| web["@lanshi17/dsh-web / ctx.web"]
  perplexity["@lanshi17/dsh-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@lanshi17/dsh-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@lanshi17/dsh-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@lanshi17/dsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@lanshi17/dsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@lanshi17/dsh-tool-web` depends on `@lanshi17/dsh-web`, `@lanshi17/dsh-tools`, `@lanshi17/dsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@lanshi17/dsh-web'
...
```yaml
- id: web
  name: '@lanshi17/dsh-web'
  config:
    searchProvider: exa
    fetchProvider: http

- id: web-search-exa
  name: '@lanshi17/dsh-web-search-exa'

- id: web-search-perplexity
  name: '@lanshi17/dsh-web-search-perplexity'

- id: web-search-deepseek
  name: '@lanshi17/dsh-web-search-deepseek'

- id: web-fetch-http
  name: '@lanshi17/dsh-web-fetch-http'
