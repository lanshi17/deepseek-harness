...
packages/    @lanshi17/dsh-<pkg> workspaces at packages/<group>/<pkg>/
...
native/      @lanshi17/node-addon-landlock-run source of record (see native/README.md)
...
- Every npm package is `@lanshi17/dsh-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@lanshi17/cordis` is a peerDependency (+ dev) of every harness package.
