...
This package implements one skill source. It scans local project, custom, and user skill roots, parses `SKILL.md` or flat Markdown skill files, and registers the provider on `ctx.skills`. The registry remains in `@lanshi17/dsh-skill`; the durable session catalogs and model-facing loader tool remain in `@lanshi17/dsh-tool-skill`.
...
| `dshHome` | `$DSH_HOME` or `~/.dsh` | DeepSeek Harness config root resolved by [`@lanshi17/dsh-home-paths`](../../util/home-paths/README.md); scans `skills` under this directory. |
