...
import type { Session, SessionEvent, SessionHeader, SessionId } from '@lanshi17/dsh-session'
import type { SessionPersistence } from '@lanshi17/dsh-session-persistence'
import type { SessionProjectionRegistry } from '@lanshi17/dsh-session-projection'
import type { SessionProjectionCache } from '@lanshi17/dsh-session-projection-cache'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @lanshi17/dsh-session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @lanshi17/dsh-session)',
