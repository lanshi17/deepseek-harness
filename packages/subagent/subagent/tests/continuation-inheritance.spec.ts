...
import SandboxPolicyService, { effectiveSandboxMode, setSandboxMode } from '@lanshi17/dsh-sandbox-policy'
...
import ApprovalService, { effectiveApprovalPolicy } from '@lanshi17/dsh-user-approval'
  ...
  it('seeds the parent sandbox override and pins approval to never', { timeout: 20_000 }, async () => {
    ...
    const runtimeContext = loaded.events.find(
      (event): event is SessionEvent<'user/message'> => event.type === 'user/message'
        ...
        && event.data.source.plugin === '@lanshi17/dsh-system-prompt',
    )
