...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@lanshi17/dsh-tools'
import { assembleContextFor, type Agent } from '@lanshi17/dsh-agent'
...
import type { SubagentStartRequest } from '@lanshi17/dsh-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @lanshi17/dsh-jobs')
  })
