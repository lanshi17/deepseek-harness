...
这是 [`@lanshi17/dsh-code-runtime`](../code-runtime/README.md) seam 的 worker 线程实现：`WorkerThreadCodeRuntime` 会在每次运行中使用一个全新的 `worker_threads.Worker`，输入 TypeScript，由宿主侧剥离类型，通过消息端口桥接绑定，输出 `{ value, logs, error? }`。**这是隔离措施，而非安全边界**：其信任立场有意与 bash 等价（参见 [Code Mode Agent Note](../../../.agents/notes/implemented/feature/2026-06-15-code-mode.md) 的 Trust posture 章节），但提供 bash 没有的隔离：独立 isolate、空环境、堆上限与强制终止。
...
- **在执行上下文中，由宿主侧剥离类型**：程序会包裹在异步函数外壳中，在导出 `node:module` 的 `stripTypeScriptTypes` 时使用它剥离类型（Node），否则改用 amaro WASM 剥离器的 `strip-only` 模式（bun 的 `node:module` polyfill 不导出任何剥离器）。两者共享本 seam 依赖的契约：只支持可擦除语法——`enum`／namespace 会作为程序 `exception` 被拒绝，且不会启动 worker——且输出保持字节位置不变，因此外壳会原样保留，函数体可按字节位置切回；之后程序作为 `AsyncFunction` 的函数体执行，因此顶层 `await`／`return` 可用。
...
源代码模式通过 Node 原生类型剥离加载只包含可擦除语法的 `src/worker.ts`。其传递运行时闭包只包含 Node 内置模块和相对源模块，因此全新 checkout 绝不需要兄弟工作区包尚未构建的 `lib/` 导出。worker 本地和会话自有的 JSON 边界都会在消息端口两侧展平并重建已验证值，使应用嵌套永远不会进入 structured clone。构建模式会把兄弟文件 `lib/worker.cjs` 作为文件系统路径传入，因为 pkg 的虚拟文件系统（VFS）Worker hook 要求 CommonJS；同一路径也可在普通 Node 与 bun（其 `worker_threads` 同样接受该路径）下使用。对这个已发布入口路径进行测试的仓库级要求由[测试策略](../../../docs/testing.md)规定。
...
- **类型剥离优先使用 `node:module` 的 `stripTypeScriptTypes`，否则使用 amaro WASM 剥离器的 `strip-only` 模式（bun）**：两者共享本 seam 依赖的同一套 strip-only 语义；amaro 正是该内置函数所包装的替代实现。
- **在 bun 下不执行 `computeMs`**：bun 的 `worker_threads` 不提供事件循环占用率（其 stub 恒报 0），因此会跳过忙碌时间轮询，只有墙钟上限与输出上限生效。
