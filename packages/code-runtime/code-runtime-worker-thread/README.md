...
Worker-thread implementation of the [`@lanshi17/dsh-code-runtime`](../code-runtime/README.md) seam: `WorkerThreadCodeRuntime` runs each program in ONE fresh `worker_threads.Worker` — TypeScript in, type-stripped host-side, bindings bridged over the message port, `{ value, logs, error? }` out. **Containment, not a security boundary**: trust posture is bash-equivalent by design (the [Code Mode Agent Note](../../../.agents/notes/implemented/feature/2026-06-15-code-mode.md) § Trust posture), with containment bash does not have — separate isolate, empty environment, heap cap, hard termination.

## Config

```yaml
- id: code-runtime
  name: '@lanshi17/dsh-code-runtime-worker-thread'
  config:
...
- **Type-strip host-side, in execution context** — the program is wrapped in an async-function shell and stripped with `node:module`'s `stripTypeScriptTypes` where that builtin is exported (Node), else with the amaro WASM stripper's `strip-only` mode (bun's `node:module` polyfill exports no stripper). Both share the contract this seam relies on: erasable syntax only — `enum`/namespaces are rejected as a program `exception` and no worker spawns — and position-preserving output, so the shell survives byte-identical and the body slices back out byte-positioned; it then executes as the body of an `AsyncFunction`, so top-level `await`/`return` work.
...
Source mode loads erasable-only `src/worker.ts` through Node's native type stripping. Its transitive runtime closure contains only Node built-ins and relative source modules, so a fresh checkout never requires a sibling workspace package's unbuilt `lib/` export. The worker-local and session-owned JSON boundaries both flatten and rebuild validated values around the message port so application nesting never reaches structured clone. Built mode passes the sibling `lib/worker.cjs` as a filesystem path because pkg's VFS Worker hook expects CommonJS; the same path works under ordinary Node and under bun (whose `worker_threads` accepts it too). The repository-wide requirement to exercise this published entry path belongs to the [testing policy](../../../docs/testing.md).
...
- **Type-strip uses `node:module`'s `stripTypeScriptTypes` where exported, else the amaro WASM stripper's `strip-only` mode (bun)** — both share the same strip-only semantics this seam relies on; amaro is the drop-in the builtin wraps.
- **Under bun, `computeMs` is not enforced** — bun's `worker_threads` exposes no event-loop utilization (its stub reports zeros), so the busy-time poll is skipped and the wall-clock ceiling plus output caps are the only budgets.
