...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@lanshi17/dsh-llm/message'
import { CallId } from '@lanshi17/dsh-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@lanshi17/dsh-attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@lanshi17/dsh-session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@lanshi17/dsh-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@lanshi17/dsh-commands/types'
import { deriveEventMessage, foldSurface } from '@lanshi17/dsh-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@lanshi17/dsh-host-apiproxy/api'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@lanshi17/dsh-tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@lanshi17/dsh-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@lanshi17/dsh-tool-read'\n" }],
  ])
