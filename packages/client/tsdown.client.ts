 ...
 * Everything else under @lanshi17/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@lanshi17\/dsh-(host-apiproxy|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@lanshi17\/dsh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@lanshi17/dsh-client-runtime/client'
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      ...
      // and every other @lanshi17 value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@lanshi17/')) return null
