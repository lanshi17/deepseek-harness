...
import type { TypertContribution } from '@lanshi17/dsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@lanshi17/dsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@lanshi17/dsh-tools')
    ...
    const record = ctx.typert.getPackage('@lanshi17/dsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@lanshi17/dsh-tools', 'host')).toBeUndefined()
  })
})
