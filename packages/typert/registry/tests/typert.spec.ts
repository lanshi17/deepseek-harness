...
} from '@lanshi17/dsh-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@lanshi17/dsh-tools', 'ToolInput')).toBe('@lanshi17/dsh-tools#ToolInput')
    expect(typertPackageKey('@lanshi17/dsh-tools', 'host')).toBe('@lanshi17/dsh-tools#host')
    expect(ctx.typert.get('@lanshi17/dsh-tools#ToolInput')).toMatchObject({
      package: '@lanshi17/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@lanshi17/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@lanshi17/dsh-tools', 'host')).toMatchObject({
      key: '@lanshi17/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@lanshi17/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@lanshi17/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@lanshi17/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@lanshi17/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@lanshi17/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@lanshi17/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@lanshi17/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@lanshi17/dsh-tools#ToolInput')).not.toBe(projected)
  })
