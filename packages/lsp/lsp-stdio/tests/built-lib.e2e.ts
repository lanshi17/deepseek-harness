...
/**
 * Keyless built-artifact smoke: plain Node imports `@lanshi17/dsh-lsp` and
 * `@lanshi17/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@lanshi17/cordis')
      const { default: Lsp } = await import('@lanshi17/dsh-lsp')
      const LspLocal = await import('@lanshi17/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@lanshi17/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@lanshi17/dsh-subprocess-local')
