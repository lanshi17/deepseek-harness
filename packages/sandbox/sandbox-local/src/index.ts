 ...
 * @module @lanshi17/dsh-sandbox-local
 */
...
} from '@lanshi17/node-addon-landlock-run'
import { Context } from '@lanshi17/cordis'
import z from '@lanshi17/schemastery'
import { assertNever } from '@lanshi17/dsh-llm'
import { SandboxProvider, SandboxUnavailableError } from '@lanshi17/dsh-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@lanshi17/dsh-sandbox'
import type { SessionId } from '@lanshi17/dsh-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@lanshi17/dsh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@lanshi17/dsh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@lanshi17/dsh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@lanshi17/dsh-sandbox-windows-acl/src/runner.ts'))
