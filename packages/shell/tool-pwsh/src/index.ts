 ...
 * `@lanshi17/dsh-pwsh-local`) backs `ctx.shell`; the tool contract is
 ...
 * exit-status parse from `@lanshi17/dsh-shell`.
 *
 * @module @lanshi17/dsh-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@lanshi17/cordis'
import z from '@lanshi17/schemastery'
import { defineTool, TOOL_ABORTED } from '@lanshi17/dsh-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@lanshi17/dsh-tools'
...
import type {} from '@lanshi17/dsh-shell-env'
import type {} from '@lanshi17/dsh-user-approval'
import type { SandboxExecutionPolicy, SandboxMode } from '@lanshi17/dsh-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@lanshi17/dsh-sandbox'
import type { SandboxPolicyService } from '@lanshi17/dsh-sandbox-policy'
import type { ShellRunResult } from '@lanshi17/dsh-shell'
import { parseExitStatus } from '@lanshi17/dsh-shell'
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @lanshi17/dsh-jobs and @lanshi17/dsh-tool-jobs')
        }
