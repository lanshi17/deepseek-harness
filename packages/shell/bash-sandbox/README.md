...
Sandbox-consuming Service Provider for the [`@lanshi17/dsh-shell`](../shell/) executor seam. Load it **instead of** `@lanshi17/dsh-bash-local`, together with a [`ctx.sandbox`](../../sandbox/sandbox/) provider (e.g. [`@lanshi17/dsh-sandbox-local`](../../sandbox/sandbox-local/)) and a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/) (which owns the default mode + workspace root, shared with the sandboxed filesystem) — no alternate tool plugin is needed; `dsh-tool-bash` detects the executor's `sandboxMode` capability and adds the escalation fields.
...
```yaml
- id: sandbox
  name: '@lanshi17/dsh-sandbox-local'
- id: sandbox-policy
...
- id: bash
  name: '@lanshi17/dsh-bash-sandbox'
```
