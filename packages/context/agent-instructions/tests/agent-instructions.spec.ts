...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@lanshi17/dsh-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@lanshi17/dsh-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@lanshi17/dsh-agent'
import AgentLoop from '@lanshi17/dsh-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@lanshi17/dsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import type {
  ToolExecution,
  ToolExecutionToken,
} from '@lanshi17/dsh-tools'
import * as ToolFs from '@lanshi17/dsh-tool-fs'
import {
  discoverBaselineInstructionFiles,
  loadBaselineInstructions,
  renderWorkspaceContext,
} from '@lanshi17/dsh-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@lanshi17/dsh-agent-instructions')
