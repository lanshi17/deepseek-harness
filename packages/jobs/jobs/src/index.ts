 ...
 * process-local registry lives in `@lanshi17/dsh-jobs-local`.
 * @module @lanshi17/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@lanshi17/dsh-jobs is the abstract job registry seam; load an implementation such as @lanshi17/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
