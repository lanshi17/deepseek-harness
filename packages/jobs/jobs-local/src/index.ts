 ...
 * @module @lanshi17/dsh-jobs-local
 */

import { Context } from '@lanshi17/cordis'
import z from '@lanshi17/schemastery'
import type { Agent } from '@lanshi17/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@lanshi17/dsh-scope'
import type { ScopeLayer } from '@lanshi17/dsh-scope'
import { deadline, timeoutOf } from '@lanshi17/dsh-timeout'
import { JobRegistry, JobId } from '@lanshi17/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@lanshi17/dsh-jobs'
 ...
 * `@lanshi17/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @lanshi17/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @lanshi17/dsh-agent)')
    }
