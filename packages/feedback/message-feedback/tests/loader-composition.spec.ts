...
import Storage from '@lanshi17/dsh-storage'
import * as StorageDomain from '@lanshi17/dsh-storage-domain'
import * as StorageJson from '@lanshi17/dsh-storage-json'
import { remoteMethods } from '@lanshi17/dsh-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@lanshi17/dsh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@lanshi17/dsh-storage', Storage],
    ['@lanshi17/dsh-storage-json', StorageJson],
    ['@lanshi17/dsh-storage-domain', StorageDomain],
    ['@lanshi17/dsh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@lanshi17/dsh-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@lanshi17/dsh-storage'",
      "- name: '@lanshi17/dsh-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@lanshi17/dsh-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@lanshi17/dsh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
