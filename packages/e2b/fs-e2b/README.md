...
E2B implementation of the [`@lanshi17/dsh-fs`](../../fs/fs/README.md) provider contract. It has no config: load [`@lanshi17/dsh-e2b`](../e2b/README.md) first, then this service in place of `dsh-fs-local`. The provider uses the owner's remote cwd and SDK handle, so file tools observe the same world as E2B-backed Bash processes.
