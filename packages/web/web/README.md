...
| Package | Role |
|---|---|
| `@lanshi17/dsh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@lanshi17/dsh-web-search-exa` | Search provider: Exa |
| `@lanshi17/dsh-web-search-perplexity` | Search provider: Perplexity |
| `@lanshi17/dsh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@lanshi17/dsh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
