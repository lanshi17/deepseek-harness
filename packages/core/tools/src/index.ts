 ...
 * execution pipeline.
 * @module @lanshi17/dsh-tools
 */

import { Context, Service } from '@lanshi17/cordis'
import z from '@lanshi17/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@lanshi17/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@lanshi17/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@lanshi17/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@lanshi17/dsh-llm'
import type { Agent } from '@lanshi17/dsh-agent'
import { snapshotJsonValue } from '@lanshi17/dsh-session'
import type { JsonValue, UserMessage } from '@lanshi17/dsh-session'
import type { ToolProviderResult } from '@lanshi17/dsh-system-prompt'
import type { CodeRuntime } from '@lanshi17/dsh-code-runtime'
...
export type { JsonValue } from '@lanshi17/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@lanshi17/dsh-tools`
...
declare module '@lanshi17/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@lanshi17/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@lanshi17/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@lanshi17/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@lanshi17/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@lanshi17/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @lanshi17/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
