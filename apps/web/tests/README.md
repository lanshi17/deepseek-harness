...
## Do not import `@lanshi17/dsh-client-*` here
...
`@lanshi17/dsh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@lanshi17/dsh-client-web` and the boot-manifest
type from `@lanshi17/dsh-client-modules/client`: booting the real shell is what
...
`@lanshi17/dsh-client-runtime/client` because `client/runtime` is reachable
