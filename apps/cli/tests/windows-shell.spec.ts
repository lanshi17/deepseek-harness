...
import { entryListSchema } from '@lanshi17/cordis-plugin-include'
import { evaluate } from '@lanshi17/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@lanshi17/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@lanshi17/dsh-base', '@lanshi17/dsh-web-app'])
    ...
    for (const name of ['@lanshi17/dsh-pwsh-sandbox', '@lanshi17/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@lanshi17/dsh-base'])
