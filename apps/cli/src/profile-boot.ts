 ...
 * @module @lanshi17/dsh/profile-boot
 */
...
import type { EntryOptions } from '@lanshi17/cordis-plugin-loader'
import {
  boot,
  composeEntries,
...
import { DSH_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@lanshi17/dsh-launch-environment'
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@lanshi17/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@lanshi17/cordis-plugin-hmr', config: { root: [] } })
      }
