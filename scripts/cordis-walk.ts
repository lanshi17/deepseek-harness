...
const MERGE_HEAD = /declare module ['"](?:@lanshi17\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@lanshi17/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@lanshi17/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@lanshi17/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@lanshi17/cordis'` Context merge
...
/**
 * Every event name a `declare module '@lanshi17/cordis'` Events merge declares in one
