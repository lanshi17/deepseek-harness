...
const SESSION_PACKAGE = '@lanshi17/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@lanshi17/dsh-session/types'
 ...
 * top-level declaration (in `@lanshi17/dsh-session`) and any declaration
 * merge inside a `declare module '@lanshi17/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @lanshi17/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
