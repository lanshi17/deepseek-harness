...
describe('release families', () => {
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@lanshi17/dsh')
    const cordis = { ...member('vendor/cordis', '@lanshi17/cordis'), version: '4.0.1' }
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@lanshi17/dsh'), { ...member('apps/web', '@lanshi17/dsh-web-frontend'), version: '0.0.2' }]
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@lanshi17/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@lanshi17/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@lanshi17/dsh-consumer', { dependencies: { '@lanshi17/dsh-library': 'workspace:^' } }),
      member('packages/a/library', '@lanshi17/dsh-library'),
      member('packages/a/zebra', '@lanshi17/dsh-zebra'),
    ]

    expect(dsh.publishOrder(members).map(entry => entry.name)).toEqual([
      '@lanshi17/dsh-library',
      '@lanshi17/dsh-consumer',
      '@lanshi17/dsh-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@lanshi17/dsh-left', { dependencies: { '@lanshi17/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@lanshi17/dsh-right', { dependencies: { '@lanshi17/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@lanshi17/dsh-library')
    const vendored = member('vendor/cordis', '@lanshi17/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@lanshi17/dsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@lanshi17/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@lanshi17/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@lanshi17/dsh-library', { files: ['lib/index.js'] }),
