...
const CHOOSER_PACKAGE = '@lanshi17/dsh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  ...
  '@lanshi17/dsh-client-ui-directory-picker-browse',
  '@lanshi17/dsh-client-ui-directory-picker-native',
]
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@lanshi17/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@lanshi17/cordis-plugin-include') return
