...
const RELEASE_ENTRY_PACKAGE = '@lanshi17/dsh'
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@lanshi17/')) {
        throw new Error(`${manifestPath} must name an @lanshi17 package`)
      }
      if (name === '@lanshi17/dsh-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@lanshi17/dsh/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@lanshi17/') || name === '@lanshi17/dsh-root')) {
