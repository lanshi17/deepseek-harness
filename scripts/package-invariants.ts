...
function checkManifest(
  ...
  if (owner.packageName === '@lanshi17/dsh-invariants') return
  if (manifest.peerDependencies?.['@lanshi17/dsh-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@lanshi17/dsh-invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['@lanshi17/dsh-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@lanshi17/dsh-invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== '@lanshi17/dsh-invariants'
