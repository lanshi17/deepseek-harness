...
import type { ToolSchema } from '@lanshi17/dsh-llm'
import AgentRegistry from '@lanshi17/dsh-agent'
import type { Agent } from '@lanshi17/dsh-agent'
import { createScope } from '@lanshi17/dsh-scope'
...
import GoalService from '@lanshi17/dsh-goal'
import SystemPrompt from '@lanshi17/dsh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@lanshi17/dsh-tools'
import LocalBashExecutor from '@lanshi17/dsh-bash-local'
import * as BashEnvPlugin from '@lanshi17/dsh-shell-env'
import { PwshLocalExecutor } from '@lanshi17/dsh-pwsh-local'
import LocalSubprocessRuntime from '@lanshi17/dsh-subprocess-local'
import LocalFileSystem from '@lanshi17/dsh-fs-local'
import { AttachmentStore } from '@lanshi17/dsh-attachment'
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@lanshi17/dsh-attachment'
import UserQuestionService from '@lanshi17/dsh-user-questions'
import PlanModeController from '@lanshi17/dsh-plan-mode'
import WebRuntime from '@lanshi17/dsh-web'
import * as WebSearchExa from '@lanshi17/dsh-web-search-exa'
import * as WebFetchLocal from '@lanshi17/dsh-web-fetch-http'
import SubagentRuntime from '@lanshi17/dsh-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@lanshi17/dsh-subagent'
import * as ToolSubagentControl from '@lanshi17/dsh-tool-subagent-control'
import * as ToolSubagentListAgents from '@lanshi17/dsh-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@lanshi17/dsh-tool-subagent-report'
import SkillRegistry from '@lanshi17/dsh-skill'
import * as SkillFileSystem from '@lanshi17/dsh-skill-filesystem'
import LocalJobRegistry from '@lanshi17/dsh-jobs-local'
import * as ToolAskUser from '@lanshi17/dsh-tool-ask-user'
import * as ToolBash from '@lanshi17/dsh-tool-bash'
import * as ToolPwsh from '@lanshi17/dsh-tool-pwsh'
import * as ToolBashPersistent from '@lanshi17/dsh-tool-bash-persistent'
import CordisHostRunner from '@lanshi17/dsh-cordis-host-runner'
import * as ToolCordis from '@lanshi17/dsh-tool-cordis'
import * as ToolFs from '@lanshi17/dsh-tool-fs'
import * as ToolFsSearch from '@lanshi17/dsh-tool-fs-search'
import * as ToolStrReplaceEditor from '@lanshi17/dsh-tool-str-replace-editor'
import TerminalSessionService from '@lanshi17/dsh-terminal'
import * as ToolPty from '@lanshi17/dsh-tool-terminal'
import * as ToolGoal from '@lanshi17/dsh-tool-goal'
import * as ToolSchedule from '@lanshi17/dsh-schedule'
import Lsp from '@lanshi17/dsh-lsp'
import * as ToolLsp from '@lanshi17/dsh-tool-lsp'
import * as ToolSkill from '@lanshi17/dsh-tool-skill'
import * as ToolSessionQuery from '@lanshi17/dsh-tool-session-query'
import * as ToolTasks from '@lanshi17/dsh-tool-jobs'
import * as ToolTodo from '@lanshi17/dsh-tool-todo'
import * as ToolSubagent from '@lanshi17/dsh-tool-subagent'
import * as ToolWeb from '@lanshi17/dsh-tool-web'
import VmWorkflowEngine from '@lanshi17/dsh-workflow-worker-thread'
import * as ToolRalph from '@lanshi17/dsh-tool-ralph'
import * as ToolWorkflow from '@lanshi17/dsh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@lanshi17/dsh-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@lanshi17/dsh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@lanshi17/dsh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@lanshi17/dsh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@lanshi17/dsh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@lanshi17/dsh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@lanshi17/dsh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@lanshi17/dsh-tool-bash-persistent',
  ...
  {
    pkg: '@lanshi17/dsh-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@lanshi17/dsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@lanshi17/dsh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@lanshi17/dsh-tool-terminal',
    dir: 'tool-terminal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@lanshi17/dsh-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@lanshi17/dsh-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@lanshi17/dsh-tool-session-query',
  ...
  {
    pkg: '@lanshi17/dsh-tool-subagent',
    dir: 'tool-subagent',
  ...
  {
    pkg: '@lanshi17/dsh-tool-subagent-control',
  ...
  {
    pkg: '@lanshi17/dsh-tool-subagent-report',
  ...
  {
    pkg: '@lanshi17/dsh-tool-workflow',
    dir: 'tool-workflow',
