...
function fixture(options: {
  ...
  const packageName = options.packageName ?? '@lanshi17/dsh-probe'
  const manifest = {
    name: packageName,
    ...
    peerDependencies: options.invariantDependency === false ? {} : {
      '@lanshi17/dsh-invariants': 'workspace:^',
    },
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) => {
  ctx.invariants.register('@lanshi17/dsh-foreign', install)
...
`
  ...
  it('rejects generated markers and reporter-free executable installers', () => {
    const generated = fixture({
      source: `/** @generated */\n${handwrittenInvariant('@lanshi17/dsh-probe')}`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) =>
  Promise.resolve(ctx.invariants.register('@lanshi17/dsh-probe', install))
`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
  ctx.invariants.register('@lanshi17/dsh-probe', () => {})
`,
    })
  ...
  it.each([
    ...
    const source = `${handwrittenInvariant('@lanshi17/dsh-probe')}\n${defaultExport}\n`
...
export const name = 'probe-invariant'
export const inject = ['invariants']
const PACKAGE_NAME = '@lanshi17/dsh-probe'
