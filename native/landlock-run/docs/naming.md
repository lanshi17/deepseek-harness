...
The public package family belongs to the `@lanshi17` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@lanshi17/node-addon-landlock-run
@lanshi17/node-addon-landlock-run-<platform>
```
